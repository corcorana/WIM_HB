...
% analysis
mform = 'y~site+stim+x+(1|sid)'; %  model formula 
nperms = 1000;
...
for sx = 1:2 % state var (1 = MS; 2 = VIG)
    ...
    for dx = 1:size(dat,1) % directionality (1=hb, 2=bh)
        ...
        [Results(sx,dx).t_obs, Results(sx,dx).betas, Results(sx,dx).se, mEEG, X, mod] = lmeEEG_fitMods(d, tab, mform);
        ...
        % apply TFCE
        for jx = 2:size(X,2)
            Results(sx,dx).(matlab.lang.makeValidName(mod.CoefficientNames{jx})) = lmeEEG_TFCE( ...
